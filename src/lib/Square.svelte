...
<div class="square position-relative">
    {#if type !== SquareType.Void}
        <Image
            src={assets.ground.src}
            alt={assets.ground.alt}
            isTransitionEnable={false}
            transitionDirection={playerDirection}
        ></Image>
        ...
        {:else if type === SquareType.Target}
            <Image
                src={assets.target.src}
                alt={assets.target.alt}
                isTransitionEnable={false}
                transitionDirection={playerDirection}
            ></Image>
        {:else if type === SquareType.Box}
            <Image
                src={assets.box.src}
                alt={assets.box.alt}
                isTransitionEnable={hasMoved}
                transitionDirection={playerDirection}
            ></Image>
        {:else if type === SquareType.BoxOnTarget}
            <Image
                src={assets.boxOnTarget.src}
                alt={assets.boxOnTarget.alt}
                isTransitionEnable={hasMoved}
                transitionDirection={playerDirection}
            ></Image>
